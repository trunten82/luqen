#!/usr/bin/env bash
# start.sh — One-command startup for Luqen (compliance + branding + dashboard)
#
# Usage:
#   ./start.sh                    # start with defaults
#   ./start.sh --pa11y-url URL    # specify pa11y webservice
#   ./start.sh --port 4000        # custom compliance port (branding = +100, dashboard = +1000)
#
# This script handles first-run setup automatically:
#   - Generates session secret (if not set)
#   - Generates JWT keys for compliance and branding (if missing)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --port)       COMPLIANCE_PORT="$2"; BRANDING_PORT=$(( $2 + 100 )); DASHBOARD_PORT=$(( $2 + 1000 )); shift 2 ;;
    --pa11y-url)  PA11Y_URL="$2"; shift 2 ;;
    --help|-h)
      echo "Usage: ./start.sh [--port PORT] [--pa11y-url URL]"
      echo ""
      echo "  --port PORT       Compliance port (default: 4000, branding = PORT+100, dashboard = PORT+1000)"
...
# ── Build (if needed) ─────────────────────────────────
if [ ! -d "packages/dashboard/dist" ] || [ ! -d "packages/compliance/dist" ] || [ ! -d "packages/branding/dist" ]; then
...
# ── Start services ────────────────────────────────────
info "Starting compliance (port ${COMPLIANCE_PORT}) + branding (port ${BRANDING_PORT}) + dashboard (port ${DASHBOARD_PORT})..."
printf "\n"

exec npx concurrently \
  -n compliance,branding,dashboard \
  -c blue,yellow,green \
