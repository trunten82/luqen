...
- **Agency tier (v3.5.0 Phase 81).** A new **Agency console** (`/admin/agency`) gives partner/agency users a multi-client overview: every organisation they can access, each with its commercial plan and managed WP-site count, plus the partner-seat entitlement. The agency plan's **partner/resale seat** (AGENCY-04) is modelled as `max_client_sites` on `org_entitlements` (migration 084) — admin-controlled (system admin only; no billing) with an over-limit warning when an account manages more sites than the seat covers. White-label reports (AGENCY-02) and per-client VPAT/ACR (AGENCY-03) are delivered by the per-org **Report Identity** (entity name + logo) already threaded through the VPAT/ACR pipeline, which the console links to and surfaces per client.
