...
CREATE INDEX IF NOT EXISTS idx_agent_messages_conv_active_created
  ...
  },
  {
    ...
    sql: `
-- Phase 37 Plan 01 (AUX-05): permalink table for org-scoped read-only
-- shares of an agent conversation snapshot. Tokens (id) are unguessable
-- base64url strings (≥128 bits of entropy) generated by ShareLinkRepository.
-- ON DELETE CASCADE on the conversation FK keeps shares from outliving
-- their target. Org membership / authz happens in the route handler
-- (Plan 04) — this layer hides revoked rows but does not gate by user.
