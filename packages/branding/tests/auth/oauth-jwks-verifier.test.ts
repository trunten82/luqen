/**
 * Phase 31.1 Plan 03 Task 2: Branding service JWKS-backed RS256 verifier.
...
describe('createJwksTokenVerifier — branding (Phase 31.1 Plan 03)', () => {
  const EXPECTED_AUD = 'https://branding.test/api/v1/mcp';
  ...
  beforeAll(async () => {
    currentKey = await makeTestKey('kid-branding-1');
    throwAwayKey = await makeTestKey('kid-throwaway-branding');
  ...
  it('Test 1: accepts a token with aud matching the expected branding MCP URL', async () => {
