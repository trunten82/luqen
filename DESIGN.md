...
**One document, two renderers, every surface.** The ACR template and stylesheet are authored once in `shared/acr/` (`acr.template.html` + `acr.css`) and rendered by *every* surface that produces an ACR — not a subset:

- **Dashboard** — `acr-view.ts` maps a built VPAT report onto the template's view shape, `acr-render.ts` renders it with mustache.js, inlines `acr.css` and the four self-hosted fonts as data URIs, and prints to PDF through headless Chromium (the browser the scanner already runs). The PDFKit path (`pdf/generator.ts`) stays only as a degrade fallback for hosts that cannot launch Chromium. A single assembly (`vpat-share-service.ts`: `buildScanAcrView` / `renderScanAcrHtml` / `renderScanAcrPdf`) backs the web report (`/reports/:id/vpat`, with an injected print/share-manager chrome), the public report (`/reports/:id/acr(.pdf)`), the token share (`/share/:token(.pdf)`), the evidence-pack PDF, the per-site report page, and the authenticated export. The old `vpat.hbs` Handlebars layout is retired.
- **WordPress plugin** — `Luqen_Vpat::build_acr_view` builds the same view shape, and a tiny dependency-free PHP port (`Luqen_Mustache`) renders the *same* template. The port matches mustache.js byte for byte: identical HTML escaping (`& < > " ' / \` =`) and the standalone-tag whitespace rule. Rendering the same view data through both renderers diffs clean (verified for `en` and `it`).

Edit the template or the stylesheet once and both surfaces change. Never fork the rendered markup into a consumer.

**No prose in the template — localizable + wording-managed.** The template carries *zero* hardcoded prose: every string comes from `{{strings.*}}`, resolved per-locale from a wording catalog with a localized standard default that an organisation may override per string. Provenance travels with each string (`standard` / `vpat-standard` / `custom`, plus a `reviewed` flag, translator and revision date), and the report shows a "standard vs custom wording" indicator. Dashboard: `acr-wording.ts` (catalog over the app i18n `vpat.*`/`acr.*` keys) + the per-org `acr_wording` store + the `/admin/acr-wording` editor (per-locale, with bulk JSON import/export). WordPress: `Luqen_Acr_Wording` (English defaults byte-identical to the dashboard's resolved `en`) + the per-site `luqen_wp_acr_wording` option + the matching wording admin page. Non-English standard wording is scaffolded for every supported locale but flagged needs-human-revision until legal sign-off; ship `en` first, render any locale.

**Content-complete + defensible.** The document includes the dated good-faith remediation record (AI-proposed-draft → developer-verified stats, with the actor on every event), the verdict-change **audit history** (criterion, from→to, reason, actor, date — reasoned changes only), the manual-test evidence appendix (images inlined, documents linked), and report links (PDF, evidence pack, live + dashboard report, badge).

**Snapshot report page + revisions.** Each completed (incl. scheduled) scan is a retained, dated revision. A stable per-site page — dashboard `/reports/live/:badgeId` (keyed on the public badge handle, no org/scan-id leak); WordPress the share-token URL — shows the latest ACR plus a browsable revisions timeline. Viewing or downloading a **non-latest** revision renders a prominent "a newer version of this report is available" disclaimer linking to the latest. The badge and the accessibility statement link to this page; the evidence pack is publicly downloadable whenever the report is exposed.
