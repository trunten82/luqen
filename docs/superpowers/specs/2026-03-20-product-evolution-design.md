# Pally Agent: Product Evolution — Composable Accessibility Platform
...
Pally Agent has grown from a single scanner into a 4-package ecosystem (core, compliance, dashboard, monitor) with 20 MCP tools, a REST API, a web dashboard, Docker/K8s infrastructure, and CI/CD pipelines. The system is technically complete and well-tested (813+ tests), but the user experience suffers from:
...
1. Position pally-agent as a **modular toolkit** where users pick the components they need.
...
- Bundling pa11y webservice into pally-agent
...
```
Tier 0: @pally-agent/core          Scan, fix, report. Zero deps on other packages.
Tier 1: @pally-agent/compliance    Legal rule engine. Standalone or enriches core.
Tier 2: @pally-agent/monitor       Regulation watcher. Standalone or feeds compliance.
Tier 3: @pally-agent/dashboard     Web UI. Consumes any combination of the above.
```
...
- `pally-agent scan https://example.com` works immediately with a pa11y webservice instance.
- WCAG criterion mapping is built-in.
- Compliance enrichment is optional: pass `--compliance-url <url>` on the CLI or set `compliance.url` in `.pally-agent.json`. New code change: also support `PALLY_COMPLIANCE_URL` env var for convenience.

**Compliance (Tier 1):**
- `npm install -g @pally-agent/compliance`
- `pally-compliance serve` starts a standalone API on port 4000.
- Pre-seeded with 58 jurisdictions and 62 regulations.
- Usable via REST, MCP, or A2A independently.

**Monitor (Tier 2):**
- `npm install -g @pally-agent/monitor`
- `pally-monitor scan` runs a one-shot check of monitored sources.
- Reads sources from compliance service when available, or from a local config file when standalone (new capability). The local config file is `.pally-monitor.json` in the current directory, containing `{ "sources": [{ "name": "...", "url": "...", "type": "html|rss|api" }] }`. Lookup order: CLI `--config` flag, then cwd, then `$HOME`.

**Dashboard (Tier 3):**
- `npm install -g @pally-agent/dashboard`
- `pally-dashboard serve` starts the web UI on port 5000.
...
Users encounter pally-agent at the simplest level and are guided toward more capability:

**Step 1: First scan (30 seconds)**
```bash
# After npm publish (future):
npx @pally-agent/core scan https://my-site.com

# Today (local install):
pally-agent scan https://my-site.com
```
Requires pa11y webservice running (see quick-scan guide for setup). Outputs report to terminal + JSON. Report footer hints: "Want legal compliance mapping? Add @pally-agent/compliance."
...
Install compliance, seed the database, re-run the scan. Report now includes jurisdiction-specific legal risk. Footer hints: "Want a web dashboard? Add @pally-agent/dashboard."
...
```bash
curl -fsSL https://pally.dev/install | sh
```
...
docs/
  getting-started/
    ...
    what-is-pally.md            Overview, architecture, how pieces fit together
...
2. **Core first-run UX** — helpful error when pa11y webservice is unreachable, with setup instructions. Add `PALLY_COMPLIANCE_URL` env var support to core config for easier compliance integration.
...
5. **Dead code cleanup** (from code review):
   ...
   - Fix `.gitignore` pattern for `pally-reports/`.
