...
- [x] **BSCORE-01**: User can see a 0-100 color contrast sub-score derived from existing WCAG 1.4.3 / 1.4.6 / 1.4.11 issues matched to brand colors
- [x] **BSCORE-02**: User can see a 0-100 typography sub-score based on brand font availability, minimum 16px body text, and line-height ≥1.5
- [x] **BSCORE-03**: User can see a 0-100 component sub-score derived from brand token vs used token set-diff (N/A when guideline has no component selectors)
- [x] **BSCORE-04**: System computes a composite overall brand score using locked `{color: 0.50, typography: 0.30, components: 0.20}` weights — not per-org customizable
- [x] **BSCORE-05**: System handles unscorable cases (no guideline linked, empty guideline, missing category data) with tagged-union score type — never `null → 0` coercion

### Persistence & Trend (BSTORE)

- [x] **BSTORE-01**: System applies migration 043 adding `brand_scores` table (nullable score columns + `coverage_profile` + `unscorable_reason`) and `organizations.branding_mode` column
- [x] **BSTORE-02**: System writes one append-only `brand_scores` row per scan completion (scanner orchestrator), including composite + 3 sub-scores + coverage profile + mode + timestamps
- [x] **BSTORE-03**: System writes one append-only `brand_scores` row per retag completion (branding-retag) without overwriting prior rows — trend preserves history
- [x] **BSTORE-04**: User can see brand score history via server-side LEFT JOIN queries that include pre-v2.11.0 scans (NULL handled as empty-state, not zero)
- [x] **BSTORE-05**: User can see brand-related issue count vs total issue count (`X of Y issues are on brand elements`) on report panel — cheap differentiator from existing matcher output
- [x] **BSTORE-06**: System does NOT backfill historical scans — pre-v2.11.0 scans render empty-state, never fake `0` scores

### Orchestrator Dual-Mode (BMODE)

- [x] **BMODE-01**: System provides a `BrandingAdapter` interface with `EmbeddedBrandingAdapter` (wraps existing in-process path) and `RemoteBrandingAdapter` (finally instantiates dormant `BrandingService`) — both return unified `BrandedIssue[]` shape
- [x] **BMODE-02**: System provides a `BrandingOrchestrator` in the dashboard that reads `orgs.branding_mode` per-request (no caching) and routes match+score calls to the correct adapter; `ServiceClientRegistry` is unchanged
- [x] **BMODE-03**: Admin user (permission `organizations.manage`) can flip per-org branding mode via org edit page with two-step confirmation and a "Reset to system default" action
- [x] **BMODE-04**: Admin user can click a test-connection button that routes through the **production** code path and returns `routedVia: 'embedded' | 'service'` + success/failure status
- [x] **BMODE-05**: System marks scans as `degraded` with `unscorable_reason` when service-mode branding is unreachable — NEVER silently reroutes to embedded; trend line renders dashed gap for degraded segments

### UI Surfaces (BUI)

- [x] **BUI-01**: User can see a Brand Score panel on the report detail page showing composite + 3 sub-scores, delta vs previous scan, brand-vs-non-brand counter, and empty-state for pre-v2.11.0 scans
- [x] **BUI-02**: User can see a Brand Score widget on the home dashboard showing big number + trend arrow + delta + inline SVG `<polyline>` sparkline (zero-JS, accessible with sr-only description)
- [x] **BUI-03**: All new UI strings (panel, widget, admin mode toggle, empty states, degraded explanations) are translated via `{{t}}` helpers across en/fr/it/pt/de/es
- [x] **BUI-04**: Branding service appears in System Health page, sidebar links, and any newly added admin sections with consistent status / badge / link patterns matching compliance and LLM services
