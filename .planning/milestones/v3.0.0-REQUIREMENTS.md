<!-- Archived: v3.0.0 shipped 2026-04-24. All 22 requirements satisfied. See v3.0.0-MILESTONE-AUDIT.md for coverage details. -->
...
- [x] **MCPI-05**: MCP Resources expose scan reports and brand scores as read-only context
- [x] **MCPI-06**: MCP Prompts expose predefined workflow shortcuts (scan, report, fix)

### MCP Service Tools

- [x] **MCPT-01**: User can scan sites, list reports, and check issues via compliance MCP tools
- [x] **MCPT-02**: User can list guidelines, get brand scores, and run discover-branding via branding MCP tools
- [x] **MCPT-03**: User can generate fixes and analyse reports via LLM MCP tools
- [x] **MCPT-04**: User can manage users, orgs, and service connections via dashboard MCP tools
- [x] **MCPT-05**: External MCP clients (Claude Desktop, IDEs) can connect and use tools with valid credentials
...
- [x] **AGENT-04**: Agent references recent scans, active guidelines, and regulations in responses
- [x] **AGENT-05**: Agent manages token budget with sliding window and summary compaction for long conversations

### Agent Persistence

- [x] **APER-01**: User's conversation history persists across sessions in SQLite
- [x] **APER-02**: State-changing tool calls require explicit user confirmation via native dialog
- [x] **APER-03**: Every tool invocation is logged with user, org, tool, args, outcome, and latency
- [x] **APER-04**: Admin can browse and filter agent audit logs in the dashboard
...
- [x] **MCPAUTH-04**: Opening an MCP connection requires the `mcp.use` RBAC permission in the user's active org (or `admin.system` globally). Tool visibility is RBAC ∩ scope defense-in-depth; admin.* OAuth scopes retired from `scopes_supported`; pre-existing tokens continue to work via silent admin.*-scope ignore (migration back-fills `mcp.use` onto all existing users per-org)
- [x] **MCPAUTH-05**: Org admins (`admin.org`) can view and revoke DCR-registered OAuth clients belonging to users within their own org via `/admin/clients`; cross-org operations return 403 + audit entry; `registered_by_user_id` is populated at first consent (RFC 7591 DCR pre-auth intent preserved)
...
| MCPI-05 | Phase 30 | Complete |
| MCPI-06 | Phase 30 | Complete |
| MCPT-01 | Phase 30 | Complete |
| MCPT-02 | Phase 29 + 30 | Complete |
| MCPT-03 | Phase 29 | Complete |
| MCPT-04 | Phase 30 | Complete |
| MCPT-05 | Phase 30 | Complete |
| AGENT-01 | Phase 32 | Complete |
| AGENT-02 | Phase 32 | Complete |
| AGENT-03 | Phase 32 | Complete |
| AGENT-04 | Phase 33 | Complete |
| AGENT-05 | Phase 33 | Complete |
| APER-01 | Phase 31 | Complete |
| APER-02 | Phase 32 | Complete |
| APER-03 | Phase 31 | Complete |
| APER-04 | Phase 33 | Complete |
| MCPAUTH-01 | Phase 31.1 | Complete |
| MCPAUTH-02 | Phase 31.1 | Complete |
| MCPAUTH-03 | Phase 31.1 | Complete |
| MCPAUTH-04 | Phase 31.2 | Complete |
| MCPAUTH-05 | Phase 31.2 | Complete |
