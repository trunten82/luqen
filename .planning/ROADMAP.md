...
- [~] **B. WordPress vision mirror — ENTERPRISE SLICE DONE 2026-06-02.** Dashboard `/scans/:id/issues` now serialises `runner` (luqen `446beda`); WP plugin v0.27.0 (`e5516fe`) flags `runner='vision'` findings with an "AI vision" badge in *Luqen → Issues* + `sanitize_runner()` now allows `'vision'` (was downgraded). Verified on wp-test lxc (PHPCS/PHPUnit 81/smoke/badge-harness/admin-page-renders). REMAINING: **standalone-mode** client-side screenshot capture → connected Luqen's `/api/v1/analyse-visual` → findings POST (gated on a configured connection; `html2canvas` vs CORS tradeoff). Full plan in `.planning/NEXT-SESSION-PROMPT.md`.
