...
- [~] **C. dashboard polish** — C#3 `analyse-visual` LLM MCP tool DONE (`0afbc49`); C#4 (admin/llm surfacing) verified already satisfied (in `CAPABILITY_NAMES`). REMAINING: C#1 alt-text vision check (needs per-image bytes in `captureVisualContext` + per-image `analyse-visual` calls; core browser tests local-only) and C#2 positive-pass VPAT upgrade ("clean vision pass → Supports" via `vpat-service.ts deriveRow`; legally sensitive — needs a vision-evaluated-criteria set plumbed from the scan).
- [ ] HUMAN UAT: TTS + image upload are UI — need real-browser cross-persona UAT ([[feedback_ui_phase_uat]]). Requires a vision model assigned to the `agent-conversation` capability on /admin/llm.
